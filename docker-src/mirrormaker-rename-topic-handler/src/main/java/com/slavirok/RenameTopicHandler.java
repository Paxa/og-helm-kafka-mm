...
public class RenameTopicHandler implements MirrorMaker.MirrorMakerMessageHandler {
    ...
    public RenameTopicHandler(String topics) {
        System.out.println("RenameTopicHandler init");
        if (topics != null && !topics.isEmpty()){
            for (String topicAssignment : topics.split(";")) {
                String[] topicsArray = topicAssignment.split(",");
                if (topicsArray.length == 2) {
                    System.out.println(String.format("RenameTopicHandler MAP %s -> %s", topicsArray[0], topicsArray[1]));
                    topicMap.put(topicsArray[0], topicsArray[1]);
                } else {
                    System.out.println(String.format("RenameTopicHandler INVALID FORMAT %s", topicAssignment));
                }
            }
        } else {
            System.out.println("RenameTopicHandler ARGUMENT MISSING");
        }
    }
